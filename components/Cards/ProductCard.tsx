...
import { ShoppingCart, DollarSign, Package, Palette, Ruler } from "lucide-react"
...
export default function ProductCard({
  ...
  const handleDelete = async () => {
    if (!confirm("هل أنت متأكد من حذف هذا المنتج؟") || isDeletingPost) return
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-2">
          ...
          {inStock ? (
            ...
            <span className="bg-red-100 text-red-800 text-xs px-2 py-1 rounded-full">غير متوفر</span>
          )}
        </div>
            ...
            {sizes.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-1 flex-wrap">
                  ...
                  {sizes.length > 5 && <span className="text-gray-500 text-xs">+{sizes.length - 5}</span>}
                </div>
              </div>
            )}
            ...
            {colors.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-1 flex-wrap">
                  ...
                  {colors.length > 5 && <span className="text-gray-500 text-xs">+{colors.length - 5}</span>}
                </div>
              </div>
            )}
          </div>
