...
export default function AdsPageClient({ session, searchParams }: PostsPageClientProps) {
  ...
  // Categories configuration for ads
  ...
  const LoadingSkeleton = () => (
    <div className="space-y-4">
      {[...Array(3)].map((_, i) => (
        <div key={i} className="overflow-hidden border shadow-sm bg-white rounded-lg animate-pulse">
          ...
          <div className="p-4">
            ...
            {/* Reactions and Action Buttons Row */}
  ...
  const updateURL = useCallback(
    (category: string, page: number) => {
      ...
      router.replace(`/main/ads${url}`, { scroll: false })
    },
    [router],
  )
  ...
  const fetchPosts = useCallback(
    async (category = "all", page = 1, isLoadMore = false) => {
      ...
      try {
        ...
        console.log(`Fetching ads: category=${category}, page=${page}, isLoadMore=${isLoadMore}`)

        const response = await fetch(`/api/main/ads?${params}`, {
        ...
        const transformedPosts =
          (data.ads || data.data)?.map((post: any) => ({
            ...
            targetAmount: post.targetAmount || undefined,
            currentAmount: post.currentAmount || undefined,
  ...
  const loadMore = useCallback(() => {
    if (!loadingMore && !isLoadingRef.current && pagination?.hasNextPage && posts.length > 0) {
      ...
      console.log(`Loading more ads: page ${nextPage}`)
  ...
  const getTotalCount = () => {
    ...
    return pagination.totalPosts || pagination.totalAds || pagination.totalIdeas || 0
  }
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-lg p-4 mb-4 border">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <label className="text-sm font-medium whitespace-nowrap">اعرض اخر إعلانات:</label>
      ...
      {pagination && (
        <div className="mb-4 text-sm text-gray-600">
          <span>
            عرض {posts.length} من أصل {getTotalCount()} إعلان
          ...
          <DialogTrigger asChild>
            <div className="border rounded-lg p-4 mb-4 cursor-pointer hover:bg-gray-50 transition-colors">
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <Input placeholder="أضف إعلان جديد" className="flex-1 cursor-pointer text-sm sm:text-base" readOnly />
              </div>
            </div>
          </DialogTrigger>
          ...
          <>
            ...
            {pagination && !pagination.hasNextPage && posts.length > 0 && (
              <div className="text-center py-8">
                <div className="bg-gray-50 rounded-lg p-6">
                  ...
                  <p className="text-gray-600 text-sm">تم عرض جميع الإعلانات</p>
                  {getTotalCount() > 0 && (
                    <p className="text-gray-500 text-xs mt-1">
                      إجمالي {getTotalCount()} إعلان
                    </p>
                  )}
                </div>
              </div>
            ...
            {error && posts.length > 0 && (
              <div className="text-center py-4">
                <p className="text-red-500 mb-2">حدث خطأ أثناء تحميل المزيد من الإعلانات</p>
          ...
          !loading && (
            <div className="text-center py-8">
              <div className="bg-gray-50 rounded-lg p-8">
                ...
                <p className="text-gray-600 text-lg mb-2">لا توجد إعلانات في هذا القسم</p>
