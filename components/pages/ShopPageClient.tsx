...
export default function ShopPageClient({ session, searchParams }: PostsPageClientProps) {
  ...
  // Categories configuration for products
      ...
      try {
        ...
        const transformedPosts =
          data.products?.map((post: any) => ({
            ...
            title: post.title || post.name || "عنوان غير محدد",
            ...
            image: post.image || post.imageUrl || undefined,
            price: post.price || "0",
            currency: post.currency || "ريال",
  ...
  const handleNewPost = useCallback(
    (newPost: any) => {
      const transformedPost = {
        ...
        title: newPost.title || newPost.name || "عنوان غير محدد",
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-lg p-4 mb-4 border">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <label className="text-sm font-medium whitespace-nowrap">اعرض اخر منتجات:</label>
      ...
      {pagination && (
        <div className="mb-4 text-sm text-gray-600">
          <span>
            عرض {posts.length} من أصل {pagination.totalPosts || pagination.totalProducts || 0} منتج
      ...
      {session && (
        ...
        >
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <Input placeholder="أضف منتج جديد للبيع" className="flex-1 cursor-pointer text-sm sm:text-base" readOnly />
          </div>
        </div>
      )}
          ...
          <>
            ...
            {pagination && !pagination.hasNextPage && posts.length > 0 && (
              <div className="text-center py-8">
                <div className="bg-gray-50 rounded-lg p-6">
                  ...
                  <p className="text-gray-600 text-sm">تم عرض جميع المنتجات</p>
                  {(pagination.totalPosts || pagination.totalProducts) && (
                    <p className="text-gray-500 text-xs mt-1">
                      إجمالي {pagination.totalPosts || pagination.totalProducts} منتج
                    </p>
                  )}
                </div>
              </div>
            ...
            {error && posts.length > 0 && (
              <div className="text-center py-4">
                <p className="text-red-500 mb-2">حدث خطأ أثناء تحميل المزيد من المنتجات</p>
          ...
          !loading && (
            <div className="text-center py-8">
              <div className="bg-gray-50 rounded-lg p-8">
                ...
                <p className="text-gray-600 text-lg mb-2">لا توجد منتجات في هذا القسم</p>
