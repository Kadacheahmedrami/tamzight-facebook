...
export default function IdeasPageClient({ session, searchParams }: PostsPageClientProps) {
  ...
  // Categories configuration for ideas
  ...
  const updateURL = useCallback(
    (category: string, page: number) => {
      ...
      router.replace(`/main/ideas${url}`, { scroll: false })
    },
    [router],
  )
  ...
  const fetchPosts = useCallback(
    async (category = "all", page = 1, isLoadMore = false) => {
      ...
      try {
        ...
        console.log(`Fetching ideas: category=${category}, page=${page}, isLoadMore=${isLoadMore}`)

        const response = await fetch(`/api/main/ideas?${params}`, {
        ...
        const transformedPosts =
          data.ideas?.map((post: any) => ({
            ...
            status: post.status || "قيد المراجعة",
            priority: post.priority || "متوسطة",
            votes: post.votes || 0,
            stats: {
        ...
        setError("فشل في تحميل الأفكار. يرجى المحاولة مرة أخرى.")
  ...
  const loadMore = useCallback(() => {
    if (!loadingMore && !isLoadingRef.current && pagination?.hasNextPage && posts.length > 0) {
      ...
      console.log(`Loading more ideas: page ${nextPage}`)
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-lg p-4 mb-4 border">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <label className="text-sm font-medium whitespace-nowrap">اعرض اخر أفكار:</label>
      ...
      {pagination && (
        <div className="mb-4 text-sm text-gray-600">
          <span>
            عرض {posts.length} من أصل {pagination.totalPosts || pagination.totalIdeas || 0} فكرة
          ...
          <DialogTrigger asChild>
            <div className="border rounded-lg p-4 mb-4 cursor-pointer hover:bg-gray-50 transition-colors">
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <Input
                  placeholder="شارك فكرة إبداعية جديدة"
          ...
          <>
            ...
            {pagination && !pagination.hasNextPage && posts.length > 0 && (
              <div className="text-center py-8">
                <div className="bg-gray-50 rounded-lg p-6">
                  ...
                  <p className="text-gray-600 text-sm">تم عرض جميع الأفكار</p>
                  {(pagination.totalPosts || pagination.totalIdeas) && (
                    <p className="text-gray-500 text-xs mt-1">
                      إجمالي {pagination.totalPosts || pagination.totalIdeas} فكرة
                    </p>
                  )}
                </div>
              </div>
            ...
            {error && posts.length > 0 && (
              <div className="text-center py-4">
                <p className="text-red-500 mb-2">حدث خطأ أثناء تحميل المزيد من الأفكار</p>
          ...
          !loading && (
            <div className="text-center py-8">
              <div className="bg-gray-50 rounded-lg p-8">
                ...
                <p className="text-gray-600 text-lg mb-2">لا توجد أفكار في هذا القسم</p>
