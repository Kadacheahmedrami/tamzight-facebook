...
export default function TruthPage() {
  ...
  const fetchTruthPosts = async (type = "all") => {
    setLoading(true)
    try {
      const url = type === "all" ? "/api/truth" : `/api/truth?type=${type}`
      const response = await fetch(url)
      const data = await response.json()
      setTruthPosts(data)
    } catch (error) {
      console.error("Error fetching truth posts:", error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const navigationItems = [
    { href: "/", icon: Home, label: "الصفحة الرئيسية", description: "العودة للصفحة الرئيسية" },
    {
      href: "/truth",
      icon: Sun,
      label: "حقائق ثابتة حول الامازيغ",
      description: "حقائق تاريخية وثقافية",
      active: true,
    },
      ...
      <div className="lg:hidden bg-gradient-to-r from-blue-600 to-blue-700 text-white p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Sun className="h-6 w-6 text-blue-200" />
            <div>
              <h1 className="font-bold text-lg">حقائق ثابتة حول الامازيغ</h1>
              <p className="text-blue-200 text-sm">حقائق تاريخية وثقافية</p>
            </div>
          </div>
          <Sheet>
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <nav className="mb-4">
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <span>حقائق ثابتة حول الامازيغ</span>
              </div>
            </nav>
            ...
            <div className="bg-white rounded-lg p-4 mb-4 border">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                <label className="text-sm font-medium whitespace-nowrap">اعرض الحقائق:</label>
                <Select value={selectedType} onValueChange={handleTypeChange}>
                  <SelectTrigger className="w-full sm:w-48">
                    <SelectValue placeholder="اختار نوع الحقائق" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">الجميع</SelectItem>
                    <SelectItem value="historical">تاريخية</SelectItem>
                    <SelectItem value="cultural">ثقافية</SelectItem>
                    <SelectItem value="geographical">جغرافية</SelectItem>
                    <SelectItem value="linguistic">لغوية</SelectItem>
                  </SelectContent>
                </Select>
                <Button size="sm" onClick={() => fetchTruthPosts(selectedType)} className="w-full sm:w-auto">
                  اعرض الحقائق
                </Button>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {truthPosts.length > 0 ? (
                truthPosts.map((post) => <PostCard key={post.id} {...post} />)
              ) : (
                <div className="text-center py-8 text-gray-500">لا توجد حقائق في هذا القسم</div>
              )}
            </div>
          </div>
        </div>
