...
export default function FriendsPage() {
  ...
  const fetchFriends = async () => {
    setLoading(true)
    try {
      // You'll need to get the userId from your auth context or props
      // For now, I'm using a placeholder - replace with actual userId
      const userId = "your-user-id-here" // Replace with actual user ID
      
      const response = await fetch(`/api/main/friends?userId=${userId}`)
      const data = await response.json()
      
      if (response.ok) {
        setFriendsData({
          friends: data.friends,
          suggestions: [] // API doesn't return suggestions, keeping empty for now
        })
      } else {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      {/* Mobile Friend Suggestions Button - Hidden since no suggestions from API */}
      ...
      {/* Mobile Friend Suggestions Section - Only show if there are suggestions */}
      {friendsData.suggestions.length > 0 && (
