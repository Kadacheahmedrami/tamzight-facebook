...
export default function ChatsClient({ session, initialConversations, initialError }: ChatsClientProps) {
  const [conversations, setConversations] = useState<Conversation[]>(initialConversations)
  const [search, setSearch] = useState("")
  const [filter, setFilter] = useState<FilterType>("all")
  const [loading, setLoading] = useState(!initialConversations.length && !initialError)
  const [error, setError] = useState<string | null>(initialError)

  // Fetch conversations from API (only if not loaded from server)
  const fetchConversations = async () => {
    if (initialConversations.length > 0) return // Already loaded from server
  ...
  useEffect(() => {
    ...
    if (!initialConversations.length && !initialError) {
      fetchConversations()
    }
  ...
  }, [initialConversations.length, initialError])
