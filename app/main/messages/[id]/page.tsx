...
export default function ChatPage({ params }: { params: { id: string } }) {
  ...
  if (isLoading) {
    return (
      <div className="flex  h-[80svh] md:h-[100svh] flex-col bg-gray-50">
        {/* Header */}
  ...
  return (
    <div className="flex h-[80svh] overflow-hidden md:h-[90svh] flex-col bg-gray-50">
      {/* Header */}
      <header className="flex items-center justify-between p-4 bg-white border-b border-gray-200 shadow-sm">
        <div className="flex items-center gap-3">
          <Button 
      ...
      >
        ...
        {messages.map((msg) => (
          ...
          >
            <div
              className={`max-w-[75%] sm:max-w-[60%] md:max-w-lg rounded-2xl px-4 py-3 shadow-sm ${
                msg.fromMe 
        ...
        <div className="flex items-end gap-3">
          <div className="flex-1 relative">
            <Input
              ...
              className="resize-none min-h-[44px] pr-4 pl-12 rounded-full border-gray-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 bg-gray-50 transition-colors"
            />
          </div>
          <Button
            onClick={sendMessage}
            disabled={!text.trim()}
            size="sm"
            className="rounded-full h-11 w-11 p-0 bg-blue-500 hover:bg-blue-600 disabled:bg-gray-300 disabled:cursor-not-allowed shadow-sm transition-all duration-200"
          >
