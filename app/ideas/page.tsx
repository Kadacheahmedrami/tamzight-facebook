...
export default function IdeasPage() {
  ...
  const navigationItems = [
    ...
    { href: "/ideas", icon: Lightbulb, label: "اقتراحات لتطوير المنصة", description: "أفكار لتحسين التجمع", active: true },
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Mobile Navigation */}
      <div className="lg:hidden bg-gradient-to-r from-yellow-600 to-yellow-700 text-white p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Lightbulb className="h-6 w-6 text-yellow-200" />
            <div>
              <h1 className="font-bold text-lg">اقتراحات لتطوير المنصة</h1>
              <p className="text-yellow-200 text-sm">أفكار لتحسين التجمع</p>
            </div>
          </div>
          <Sheet>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" className="text-white hover:bg-yellow-700">
            ...
            <SheetContent side="right" className="w-80 p-0 overflow-y-auto max-h-screen">
              <div className="flex flex-col min-h-full">
                ...
                <div className="flex-shrink-0 bg-gradient-to-b from-yellow-600 to-white p-6 pt-16 text-center">
                  ...
                  <p className="text-yellow-100 text-sm">منصة التواصل الأمازيغية</p>
                </div>
                    ...
                    {navigationItems.map((item) => (
                      <Link
                        ...
                        className={`flex items-center gap-3 p-3 rounded-lg transition-all duration-200 hover:translate-x-1 group ${
                          item.active ? "bg-yellow-50 border-l-4 border-yellow-500" : "hover:bg-yellow-50"
                        }`}
                      >
                        <div
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className={`font-medium text-gray-900 truncate ${item.active ? "text-yellow-900" : ""}`}>
                            {item.label}
                          </h3>
                          <p className={`text-sm text-gray-500 truncate ${item.active ? "text-yellow-600" : ""}`}>
                            {item.description}
                          </p>
                        </div>
                        <ChevronRight
                          className={`h-4 w-4 transition-colors ${
                            item.active ? "text-yellow-500" : "text-gray-400 group-hover:text-yellow-500"
                          }`}
                        />
                      </Link>
                    ))}
