...
// GET /api/main/ideas/[id] - Get a specific idea with full details
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const userLike = currentUser ? idea.likes.find(like => like.userId === currentUser.id) : null;

    // Group reactions by emoji
    const reactionSummary = idea.likes.reduce((acc: any, like) => {
      if (like.emoji) {
    ...
    return NextResponse.json({
      idea: {
        ...
        userLike,
        reactionSummary,
        isOwner: currentUser?.id === idea.authorId
      }
    });
  } catch (error) {
...
// POST /api/main/ideas/[id] - Handle likes, reactions, shares, comments
  ...
  try {
    ...
    if (action === 'like') {
      ...
      if (existingLike) {
        ...
        // Create notification for idea author (but not for own ideas)
        const idea = await prisma.idea.findUnique({
          where: { id },
        ...
        if (idea && idea.authorId !== user.id) {
          await prisma.notification.create({
            data: {
              ...
              message: emoji
                ? `${user.firstName} ${user.lastName} reacted ${emoji} to your idea`
                : `${user.firstName} ${user.lastName} liked your idea`,
        ...
        return NextResponse.json({
          success: true,
          message: emoji ? 'Reaction added' : 'Idea liked',
          action: 'liked'
        });
      }
    }
    ...
    if (action === 'share') {
      // Check if user already shared this idea
      ...
      if (idea && idea.authorId !== user.id) {
        await prisma.notification.create({
          data: {
            userId: idea.authorId,
            type: 'share',
            message: `${user.firstName} ${user.lastName} shared your idea`,
            avatar: user.image || user.avatar
          }
        });
      }
      ...
      if (idea && idea.authorId !== user.id) {
        await prisma.notification.create({
          data: {
            userId: idea.authorId,
            type: 'comment',
            message: `${user.firstName} ${user.lastName} commented on your idea`,
            avatar: user.image || user.avatar
          }
        });
      }
