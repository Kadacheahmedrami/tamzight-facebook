...
// GET /api/main/sentences/[id]/pronunciations - Get all pronunciations for a sentence
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Get all pronunciations for this sentence
    const pronunciations = await prisma.sentencePronunciation.findMany({
      where: { sentenceId: id },
      include: {
        user: {
          select: {
    ...
    console.error('Error fetching sentence pronunciations:', error);
    return NextResponse.json(
      { error: 'حدث خطأ في جلب نطقات الجملة' },
      { status: 500 }
    );
  }
}

// POST /api/main/sentences/[id]/pronunciations - Add a new pronunciation
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error('Error adding sentence pronunciation:', error);
    return NextResponse.json(
      { error: 'حدث خطأ في إضافة نطق الجملة' },
      { status: 500 }
    );
  }
}